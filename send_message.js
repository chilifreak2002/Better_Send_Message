module.exports = {
  ...
  html(isEvent, data) {
    return `
<script>
  // Initialize container visibility on load
  window.addEventListener('DOMContentLoaded', function() {
    // Initialize button container
    const buttonMode = document.getElementById('mode');
    const authorOnlyContainer = document.getElementById('authorOnlyContainer');
    if (buttonMode && authorOnlyContainer) {
      authorOnlyContainer.style.display = buttonMode.value === 'PERSISTENT' ? 'block' : 'none';
    }
    
    // Initialize select menu containers
    document.querySelectorAll('[id="mode"].round').forEach(selectElement => {
      const container = selectElement.closest('.dialog-list-element')?.querySelector('#selectAuthorOnlyContainer');
      if (container) {
        container.style.display = selectElement.value === 'PERSISTENT' ? 'block' : 'none';
      }
    });
  });
</script>
...
<tab-system style="margin-top: 20px;">
      ...
      <dialog-list id="buttons" fields='["name", "type", "id", "row", "url", "emoji", "disabled", "mode", "authorOnly", "time", "actions"]' dialogTitle="Button Info" dialogWidth="600" dialogHeight="700" listLabel="Buttons" listStyle="height: calc(100vh - 350px);" itemName="Button" itemCols="4" itemHeight="40px;" itemTextFunction="data.name" itemStyle="text-align: center; line-height: 40px;">
        <div style="padding: 16px;">
          <div style="width: calc(50% - 12px); float: left;">
            ...
            <select id="mode" class="round" onchange="document.getElementById('authorOnlyContainer').style.display = this.value === 'PERSISTENT' ? 'block' : 'none';">
            ...
            <div id="authorOnlyContainer" style="display: none;">
              <dbm-checkbox id="authorOnly" label="Command Author Only" defaultValue="false"></dbm-checkbox>
            </div>
          </div>
          <div style="width: calc(50% - 12px); float: right;">
            ...
            <span class="dbminputlabel">Temporary Time-Limit (Seconds)</span>
            <input id="time" placeholder="60" class="round" type="text">
            <p style="margin-top: 4px; font-size: 12px;">Enter 0 for unlimited time</p>
          </div>
      ...
      <dialog-list id="selectMenus" fields='["placeholder", "id", "tempVarName", "row", "min", "max", "mode", "authorOnly", "time", "options", "actions"]' dialogTitle="Select Menu Info" dialogWidth="800" dialogHeight="700" listLabel="Select Menus" listStyle="height: calc(100vh - 350px);" itemName="Select Menu" itemCols="1" itemHeight="40px;" itemTextFunction="data.placeholder + '<br>' + data.options" itemStyle="text-align: left; line-height: 40px;">
        <div style="padding: 16px;">
          <div style="width: calc(33% - 16px); float: left; margin-right: 16px;">
            ...
            <select id="mode" class="round" onchange="const container = this.closest('.dialog-list-element').querySelector('#selectAuthorOnlyContainer'); if(container) container.style.display = this.value === 'PERSISTENT' ? 'block' : 'none';">
              <option value="PERSONAL">Once, Command User Only</option>
              <option value="PUBLIC">Once, Anyone Can Use</option>
              <option value="MULTIPERSONAL">Multi, Command User Only</option>
              <option value="MULTI" selected>Multi, Anyone Can Use</option>
            ...
            <div id="selectAuthorOnlyContainer" style="display: none;">
              <dbm-checkbox id="authorOnly" label="Command Author Only" defaultValue="false"></dbm-checkbox>
            </div>
          </div>
          <div style="width: calc(33% - 16px); float: left; margin-right: 16px;">
  ...
  init() {
    // Handle showing/hiding authorOnly checkbox for buttons
    const setupButtonContainer = () => {
      ...
      if (buttonMode) {
        ...
        const updateVisibility = () => {
          if (authorOnlyContainer) {
            authorOnlyContainer.style.display = buttonMode.value === 'PERSISTENT' ? 'block' : 'none';
          }
        };
        
        // Add event listener
        buttonMode.addEventListener('change', updateVisibility);
    ...
    setTimeout(setupButtonContainer, 100);
    
    // Add visibility handler for select menu mode changes
    const observer = new MutationObserver((mutations) => {
      document.querySelectorAll('[id="mode"].round').forEach(selectElement => {
        // Add change event listener
        selectElement.addEventListener('change', function() {
          const container = this.closest('.dialog-list-element').querySelector('#selectAuthorOnlyContainer');
          if (container) {
            container.style.display = this.value === 'PERSISTENT' ? 'block' : 'none';
          }
        });
        
        // Fire the event for initial state
    ...
    if (Array.isArray(data.buttons)) {
      for (let i = 0; i < data.buttons.length; i++) {
        ...
        if (button.mode !== "PERSISTENT") {
          const timeValue = parseInt(this.evalMessage(button.time, cache));
          // Convert seconds to milliseconds, or use default time (60 seconds)
          const timeInMs = timeValue ? timeValue * 1000 : defaultTime;
    ...
    if (Array.isArray(data.selectMenus)) {
      for (let i = 0; i < data.selectMenus.length; i++) {
        ...
        if (select.mode !== "PERSISTENT") {
          const timeValue = parseInt(this.evalMessage(select.time, cache));
          // Convert seconds to milliseconds, or use default time (60 seconds)
          const timeInMs = timeValue ? timeValue * 1000 : defaultTime;
  ...
  mod(DBM) {
    ...
    // Store the original preformActionsFromInteraction function
    const originalPreformActionsFromInteraction = Actions.preformActionsFromInteraction;
    
    // Override the function to check for authorOnly property
    Actions.preformActionsFromInteraction = function(interaction, interactionData, meta = null, initialTempVars = null) {
      // Check if this is a persistent button or select menu with authorOnly enabled
      if (interactionData.mode === "PERSISTENT" && interactionData.authorOnly === true) {
        // Get the command author's ID from the message's interaction
        const originalCommandUser = interaction.message?.interaction?.user?.id;
        
        // Compare with current user
        if (originalCommandUser && originalCommandUser !== interaction.user.id) {
